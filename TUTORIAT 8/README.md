...
![](./images/b-tree/B-tree_structure.png)
...
### <ins>1.2 - Search</ins>
![](./images/b-tree/B-tree_search.png)
...
![](./images/b-tree/B-tree_splitting.png)
- Totusi, ce ne facem cand cele 2 noduri nou obtinute il vor strica pe parintele nodului spart (prin mediana adaugata in parinte)? Din nou splitting...
- Ca urmare concepem urmatorul algoritm: plecam din radacina si vom parcurge arborele in stilul cautarii de mai sus, iar in cazul in care dam de **full nodes** le vom da split, chiar daca s-ar putea sa nu fie nevoie. Parcurgem in acest fel arborele pana ajungem la frunze (sau unde avem nevoie), in care vom insera cheia noastra; de asemenea, in cazul in care radacina este si ea full, ii vom da split si acesteia alocand un alt nod ce va reprezenta noua radacina
![](./images/b-tree/B-tree_splitting_algorithm.png)
![](./images/b-tree/B-tree_insert_algorithm.png)
![](./images/b-tree/B-tree_insert.png)
...
- **Exercitiu**: Se pastreaza proprietatea inaltimii i.e. toate frunzele se afla la aceeasi inaltime in arbore?
...
- Stergerea dintr-un B-Tree este un pic mai complicata. Pe scurt, ea consta din urmatoarele cazuri:
  ...
  - Cazul 2: Daca cheia $k$ se afla in nodul $x$, iar $x$ nu este frunza (**internal node**):
    - a: Fie nodul $y$ fiul nodului $x$ pentru care $k$ este limita superioara si are cel putin $t$ chei. Aflam predecesorul cheii $k$ din subarborele a carui radacina este $y$, fie acesta $m$ (vom parcurge arborele pana la frunze, mereu alegand cel mai din dreapta fiu). In continuare, dupa ce l-am aflat, il vom sterge pe $m$ din toate nodurile aplicand algoritmul de stergere prezentat aici (cazul 3) si apoi il vom **inlocui** pe $k$ cu $m$.
...
![](./images/b-tree/B-tree_delete.png)
![](./images/b-tree/B-tree_delete_2.png)
...
- Fiind dat un graf neorientat ponderat (muchiile au costuri), apare problema determinarii unui **APCM (arbore partial de cost minim) / MST (minimum spanning tree)** = un arbore care contine toate nodurile grafului si are suma ponderilor muchiilor minima. Evident, problema se aplica si in cazul grafurilor orientate (dar acum ne axam pe cele neorientate).
- Exista doi pasi principali in determinarea unui MST:
    - **Evitarea ciclurilor**: este posibil ca muchia selectata la pasul curent sa formeze un ciclu.
    - **Selectia optima a muchiilor**: la un pas putem avea mai multe muchii la dispozitie dintre care sa selectam - trebuie sa gasim varianta optima.
- Un graf poate avea mai multe MST-uri (cel mai simplu exemplu: fiecare muchie are aceeasi pondere).
- Mai departe vor fi prezentati **algoritmul lui Kruskal** si **algoritmul lui Prim**, folositi in rezolvarea acestei probleme. Exista doua diferente majore intre acesti doi algoritmi:
  - **Kruskal** realizeaza o prelucrare a datelor, in timp ce **Prim** incepe direct constructia MST-ului.
  - **Prim** are un mod mai simplu/indirect de a detecta cicluri, in timp ce **Kruskal** verifica mai riguros si necesita mai multa atentie.
- Mai departe prezentam doua metode pentru detectarea ciclurilor pentru **Kruskal**.
...
- Cand adaugam o muchie, verificam daca extremitatile ei fac parte din aceeasi componenta colorata. In caz afirmativ, muchia formeaza un ciclu. Altfel, cele doua extremitati apartin unor componente colorate diferite, care vor fi unite sa formeze o singura componenta.
- Motivul pentru care este ineficienta abordarea: cand vrem sa unim doua componente colorate (consideram ca prima componenta primeste culoarea celeilalte componente), trebuie sa iteram prin tot graful pentru a gasi toate nodurile care fac parte din prima componenta, recolorandu-le. **Complexitate O(n)**.
...
- O alternativa pentru metoda discutata anterior este utilizarea a doi vectori auxiliari, care simuleaza un arbore. Fiecare nod **i** are urmatoarele etichete:
  - **parent[i]**: radacina (reprezentantul) arborelui din care face parte nodul. Aici intra un truc: fiecare arbore va incerca sa aproximeze cat mai bine structura de o radacina cu numar nelimitat de copii, pentru ca arborele sa fie de inaltime minima. Astfel, daca incepem cautarea reprezentantului dintr-un nod care se afla departe de radacina, la final vom trece nodul respectiv ca si copil direct al radacinii, pentru a scurta drumul pentru cautarile din viitor.
  - **height[i]**: aceasta proprietate este actualizata numai pentru reprezentanti si reprezinta inaltimea arborelui pe care il reprezinta.
...
- Este un algoritm de tip **greedy**, deoarece mai intai se realizeaza o prelucrare a datelor, iar apoi formam solutia finala (optimul global) selectand pe rand muchii (optim local) dupa un anumit criteriu.
- Observatie: am inclus codul C++ pentru acest algoritm la fisiere.
- **Pasul 1**: Sortam muchiile crescator dupa pondere.
- **Pasul 2**: Initial, solutia (multimea de muchii pentru MST) este goala. Iteram prin multimea sortata de muchii: daca muchia curenta nu formeaza un ciclu, o adaugam in solutie; altfel, o ignoram. Algoritmul se incheie cand avem **n-1** muchii.
...
- Observatie: am inclus codul C++ pentru acest algoritm la fisiere.
- **Pasul 1**: Consideram ca avem doua multimi de noduri: multimea care formeaza APCM-ul construit pana in momentul curent, si multimea cu restul nodurilor (neincluse in APCM). Initial, alegem in mod aleator un singur nod pe care il includem in APCM.
- **Pasul 2**: La fiecare pas (pana cand am inclus toate nodurile din graf), selectam muchia de cost minim care uneste un nod ce se afla in APCM cu un nod care nu se afla in APCM. Astfel, ar fi util un priority queue.
- **Complexitate timp O((n + m)logn)**: fara explicatii ca nu am stat sa inteleg complexitatea (si gen cui ii pasa)
