...
# Table of contents
- [1 - Algoritmi si complexitati](#1---algoritmi-si-complexitati)
...
## 1 - Algoritmi si complexitati
- Structurile de date se bazeaza pe diversi algoritmi; pentru a putea analiza performanta structurilor de date, trebuie mai intai sa putem analiza algoritmii.
- Cele mai comune metrici sunt timpul de executie si memoria utilizata, ce se transpun in complexitate de timp si de spatiu.
- Pentru un algoritm oarecare si un input de dimensiune $n$, fie $f(n)$ numarul de operatii necesare finalizarii executiei. Avem urmatoarele clase de complexitati de timp:
	1. **Big O:** pentru o functie $g(n)$, avem $f(n) \in O(g(n)) \iff \exists c, n_0 \gt 0 \space astfel \space incat \space 0 \le f(n) \le cg(n), \forall n \ge n_0$
![](./images/bigO.png)
